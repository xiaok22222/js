...
!(async () => {
  ...
  $.index = 0;
  for (let i = indexLast; i < ReadArr.length; i++) {
    if (ReadArr[i]) {
      articlebody = ReadArr[i];
      var time1 = Date.parse(new Date()).toString();
                 time1 = time1.substr(0, 10);
      doubleCookie = '&device_brand=xfdg&device_id=cc7dgdsgfsz83e&device_model=1gx&device_platform=android&device_type=android&inner_version=202107261526&mi=0&openudid=cc7dgdsgfsz83e&os_api=27&os_version=bdftgsdfga&phone_network=WIFI&phone_sim=1' + '&request_time=' + time1 + '&' + articlebody
...
function double_score() {
  return new Promise((resolve) => {
    let url = {
      url: 'https://kandian.wkandian.com/v17/NewTask/setJoinTaskPromotion.json?' + doubleCookie,
